...
TRADE_CONFIG = {
    ...
    'analysis_periods': {
        'very_short_term': 5,   # 超短期均线（捕捉快速变化）
        'short_term': 20,      # 短期均线
        'medium_term': 50,     # 中期均线
        'long_term': 96,       # 长期趋势
        'trend_confirmation': 200  # 趋势确认均线
    },
    # 新增智能仓位参数
    'position_management': {
        'enable_intelligent_position': True,  # 🆕 新增：是否启用智能仓位管理
        'base_usdt_amount': 100,  # USDT投入下单基数
        'high_confidence_multiplier': 2.5,    # 🔥 HIGH信心大幅提升
        'medium_confidence_multiplier': 1.2,  # 🔥 MEDIUM信心适度提升
        'low_confidence_multiplier': 0.3,     # 🔥 LOW信心更保守
        'max_position_ratio': 0.8,  # 单次最大仓位比例（80%的账户余额）
        'trend_strength_multiplier': 1.8      # 🔥 强势趋势大幅提升
    }
}
...
def calculate_intelligent_position(signal_data, price_data, current_position):
    ...
    try:
        ...
        # BUY+HIGH信心情况下，放宽RSI超买限制
        if (signal_data['signal'] == 'BUY' and 
            signal_data['confidence'] == 'HIGH' and 
            rsi > 75):
            # 强势上涨中，RSI超买不减仓
            rsi_multiplier = 1.0
            print(f"🎯 BUY+HIGH信心信号，忽略RSI超买({rsi:.1f})不减仓")
        ...
        # 🆕 新增：根据市场状态动态调整风险参数
        market_state = detect_market_state(price_data['technical_data'], price_data['trend_analysis'])
        
        # 根据市场状态调整风险倍数
        if '高波动' in market_state:
            # 高波动市场：降低仓位，增加风险控制
            volatility_multiplier = 0.7
            max_position_ratio = config['max_position_ratio'] * 0.8  # 降低最大仓位比例
        elif '中波动' in market_state:
            volatility_multiplier = 0.9
            max_position_ratio = config['max_position_ratio'] * 0.9
        else:
            volatility_multiplier = 1.0
            max_position_ratio = config['max_position_ratio']

        # 🆕 新增：BUY+HIGH信心特殊处理 - 不减仓逻辑
        if (signal_data['signal'] == 'BUY' and 
            signal_data['confidence'] == 'HIGH' and 
            ...
            # 计算当前仓位对应的USDT价值
            current_position_usdt = current_position['size'] * price_data['price'] * TRADE_CONFIG['contract_size']
            
            # 如果建议仓位小于当前仓位，但差距不大（在20%以内），保持当前仓位
            if suggested_usdt < current_position_usdt and \
               (current_position_usdt - suggested_usdt) / current_position_usdt < 0.2:
                print(f"🔒 BUY+HIGH信心信号，小幅减仓被阻止，保持当前仓位")
                return current_position['size']
                
            # 如果建议仓位大于当前仓位，正常加仓
            elif suggested_usdt > current_position_usdt:
                print(f"🎯 BUY+HIGH信心信号，执行加仓操作")

        # 🆕 新增：根据趋势强度进一步调整
        trend_score = price_data['trend_analysis'].get('trend_score', 0)
        if trend_score >= 5:  # 强趋势
            trend_strength_multiplier = 1.2
        elif trend_score >= 3:  # 中等趋势
            trend_strength_multiplier = 1.0
        else:  # 弱趋势或无趋势
            trend_strength_multiplier = 0.8

        # 计算建议投入USDT金额（包含市场状态调整）
        suggested_usdt = base_usdt * confidence_multiplier * trend_multiplier * rsi_multiplier * volatility_multiplier * trend_strength_multiplier

        # 风险管理：不超过总资金的指定比例 - 使用动态调整的最大仓位比例
        max_usdt = usdt_balance * max_position_ratio
        ...
        print(f"   - 波动率倍数: {volatility_multiplier}")
        print(f"   - 趋势强度倍数: {trend_strength_multiplier}")
        print(f"   - 市场状态: {market_state}")
        print(f"   - 趋势强度: {trend_score}/6分")
        print(f"   - 最大仓位比例: {max_position_ratio:.2%}")
...
def calculate_technical_indicators(df):
    """计算技术指标 - 增强版，包含动量指标和波动率分析"""
    try:
        # 移动平均线 - 增强多时间框架分析
        df['sma_5'] = df['close'].rolling(window=5, min_periods=1).mean()
        df['sma_20'] = df['close'].rolling(window=20, min_periods=1).mean()
        df['sma_50'] = df['close'].rolling(window=50, min_periods=1).mean()
        df['sma_200'] = df['close'].rolling(window=200, min_periods=1).mean()  # 长期趋势确认
        ...
        # 动量指标 - 增强多周期分析
        df['momentum_5'] = df['close'].pct_change(5)    # 5周期动量（短期）
        df['momentum_10'] = df['close'].pct_change(10)  # 10周期动量（中期）
        df['momentum_20'] = df['close'].pct_change(20)  # 20周期动量（长期）
        df['roc_5'] = ((df['close'] - df['close'].shift(5)) / df['close'].shift(5)) * 100   # 5周期价格变化率
        df['roc_10'] = ((df['close'] - df['close'].shift(10)) / df['close'].shift(10)) * 100  # 10周期价格变化率
        df['roc_20'] = ((df['close'] - df['close'].shift(20)) / df['close'].shift(20)) * 100  # 20周期价格变化率
        
        # ATR（平均真实范围）用于波动率测量 - 新增
...
def get_market_trend(df):
    ...
    try:
        ...
        # 动量确认 - 增强多周期分析
        momentum_5_positive = df['momentum_5'].iloc[-1] > 0
        momentum_10_positive = df['momentum_10'].iloc[-1] > 0
        momentum_20_positive = df['momentum_20'].iloc[-1] > 0
        roc_5_positive = df['roc_5'].iloc[-1] > 0
        roc_10_positive = df['roc_10'].iloc[-1] > 0
        roc_20_positive = df['roc_20'].iloc[-1] > 0
        volume_confirmation = df['volume_ratio'].iloc[-1] > 1.2

        # 计算趋势强度分数 - 增强版（10分制）
        trend_score = sum([
            1 if trend_short == "上涨" else 0,
            1 if trend_medium == "上涨" else 0,
            1 if current_price > df['sma_200'].iloc[-1] else 0,  # 200日均线确认
            1 if macd_trend == "bullish" else 0,
            1 if momentum_5_positive else 0,
            1 if momentum_10_positive else 0,
            1 if momentum_20_positive else 0,
            1 if roc_5_positive else 0,
            1 if roc_10_positive else 0,
        ...
        return {
            ...
            'trend_score': trend_score,  # 新增
            'momentum_positive': momentum_positive,  # 新增
            'volume_confirmation': volume_confirmation  # 新增
        }
    except Exception as e:
        print(f"趋势分析失败: {e}")
        return {}
...
def detect_market_state(price_data):
    """检测市场状态 - 新增函数"""
    try:
        tech_data = price_data.get('technical_data', {})
        volatility = tech_data.get('atr_percent', 0)
        
        if volatility > 3.0:  # 3%波动率
            state = "高波动"
        elif volatility > 1.5:
            state = "中波动"
        else:
    ...
    try:
        ...
        return {
            ...
            'technical_data': {
                ...
                'roc': current_data.get('roc', 0),
                'atr': current_data.get('atr', 0),
                'atr_percent': current_data.get('atr_percent', 0)
            },
...
def generate_technical_analysis_text(price_data):
    ...
    - 趋势强度: {trend.get('trend_score', 0)}/6分

    📊 动量指标:
    - RSI: {safe_float(tech['rsi']):.2f} ({'超买' if safe_float(tech['rsi']) > 70 else '超卖' if safe_float(tech['rsi']) < 30 else '中性'})
    - MACD: {safe_float(tech['macd']):.4f}
    - 信号线: {safe_float(tech['macd_signal']):.4f}
    - 10周期动量: {safe_float(tech['momentum']):.2%}
    - 价格变化率: {safe_float(tech['roc']):.2f}%

    📈 波动率分析:
    - ATR: {safe_float(tech['atr']):.2f}
    - ATR百分比: {safe_float(tech['atr_percent']):.2f}% ({'高波动' if safe_float(tech['atr_percent']) > 3.0 else '中波动' if safe_float(tech['atr_percent']) > 1.5 else '低波动'})
    ...
    """
    # 添加市场状态检测
    market_state = detect_market_state(tech, trend)
    analysis_text += f"""
